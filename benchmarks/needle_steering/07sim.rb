...
1.times do
  seed = rand(100000000)
  %w[needle_steering_channel_same_penalty].each do |pg_name|
    [2].each do |method|
      [[1, 0]].each do |separate_planning_first, simultaneous_planning|
        ...
        #Record.create! exp_options.merge command: command, result: result, pg_name: pg_name, problem: :needle_steering, version: 36, description: "11 channels test"
      end
    end
  end
end
