...
selectorr = Record.where(version: 10701)
...
twistcosts = []
ccosts = []
  ["needle_steering_10701_sep", "needle_steering_10701_same"].each do |pg_name|
    [1, 2].each do |method|
      [[0, 1], [1, 0]].each do |separate_planning_first, simultaneous_planning|
        ...
        total_path_costs = needles.where(range).pluck(:path_length_costs).map(&:sum).inject(:+)
        total_twist_costs = needles.where(range).pluck(:twist_costs).map(&:sum).inject(:+)
        puts needles.where(range).pluck(:clearance_costs).first
        total_clearance_costs = needles.where(range).pluck(:clearance_costs).map(&:max).inject(:+)
        ...
        puts "avg path cost: #{(total_path_costs*1.0/cnt).round(3)}"
        puts "avg twist cost: #{(total_twist_costs*1.0/cnt).round(3)}"
        puts "avg clearance cost: #{(total_clearance_costs*1.0/cnt).round(3)}"
        runtime << (total_run_time*1.0/cnt).round(3)
        convcnt <<  (converged_cnt*1.0/cnt).round(3)
        pathcosts << (total_path_costs*1.0/cnt).round(3)
        twistcosts << (total_twist_costs*1.0/cnt).round(3)
        ccosts << (total_clearance_costs*1.0/cnt).round(3)
