...
selectorr = Record.where(version: 10503)
  ...
  ["needle_steering_same_penalty"].uniq.each do |pg_name|
    [1, 2].each do |method|
      [[1, 0]].each do |separate_planning_first, simultaneous_planning|
        ...
        #puts needles.where(range).where(converged:true).pluck(:twist_costs).map(&:sum).sum / converged_cnt
        #puts needles.where(range).where(converged:true).pluck(:path_length_costs).map(&:sum).sum / converged_cnt

        #puts total_path_costs / converged_cnt
