...
selectorr = Record.where(version: 36)
...
pathcosts = []
ccosts = []
  ["needle_steering_channel_same_penalty"].each do |pg_name|
    [1, 2].each do |method|
      [[0, 1], [1, 0]].each do |separate_planning_first, simultaneous_planning|
        ...
        total_path_costs = needles.where(range).pluck(:twist_costs).map(&:sum).sum + needles.where(range).pluck(:path_length_costs).map(&:sum).sum
        total_clearance_costs = needles.where(range).pluck(:clearance_costs).map(&:sum).sum 
